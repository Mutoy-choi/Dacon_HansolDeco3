...
**건설 현장의 과거 사고 사례(`database.csv`)를 기반으로, FAISS를 이용한 빠른 유사 사례 검색과 로컬 LLM(Ollama)을 활용하여 최적의 대응 방안을 제안합니다. 사용자 피드백(`responses.jsonl`)을 통해 지속적으로 개선되는 지능형 시스템입니다.**
...
본 시스템은 **RAG (Retrieval-Augmented Generation)** 아키텍처를 기반으로 건설 현장 사고 발생 시 신속하고 효과적인 대응 방안을 지원합니다. 사용자가 **사고의 주요 내용(객체, 원인 등)**을 입력하면, 시스템은 먼저 **FAISS(Facebook AI Similarity Search)**를 이용하여 `database.csv`에 저장된 방대한 **과거 유사 사고 사례** 중 관련성이 높은 내용을 효율적으로 검색합니다.

검색된 사례와 사용자 입력을 바탕으로, 로컬 환경에서 구동되는 **LLM(Ollama)**이 문맥을 이해하고 **두 가지 초기 대응 방안**을 생성하여 사용자에게 제시합니다. 사용자는 두 방안을 비교 검토 후 **더 적절한 방안을 선택**하거나, 필요에 따라 **직접 내용을 수정**하여 최적의 대응 방안을 확정할 수 있습니다.

이 과정에서 사용자의 선택 및 수정 내용은 `responses.jsonl` 파일에 **JSON Lines 형식으로 기록**되어 축적됩니다. 이 데이터는 향후 **RLHF(Reinforcement Learning from Human Feedback)**와 유사한 방식으로 LLM 모델을 파인튜닝하거나 RAG 시스템 자체를 개선하는 데 활용될 수 있습니다. 또한, 사용자는 대응 방안 검토 중 관련 **건설안전 지침 문서를 시스템 내에서 즉시 조회**하여 참고할 수 있습니다.
...
*   **고효율 유사 사례 검색**: FAISS를 활용하여 대규모 데이터에서도 빠른 속도로 관련 사고 사례 검색.
*   **듀얼 답변 제시 및 사용자 피드백**: 두 가지 대응 방안 옵션 제공 및 사용자 선택/수정 기능을 통한 시스템 개선 데이터 확보 (`responses.jsonl`).
*   **로컬 LLM 활용 (Ollama)**: 데이터 프라이버시를 보호하고 외부 API 비용 없이 강력한 LLM 기능 활용. RTX 3090과 같은 GPU 환경에서 가속화 가능.
*   **데이터 관리 스크립트**: RAG DB 구축(`buildRAGdb.py`), 업데이트(`updateRAG.py`), 정리(`cleanDB.py`)를 위한 유틸리티 스크립트 제공.
*   **참고 문서 접근성**: 시스템 내에서 관련 건설 안전 지침 문서 즉시 조회 기능.
*   **최신 개발 환경**: Python 3.12, React(Vite), FastAPI 등 현대적인 기술 스택 활용.
*   **간편한 배포**: Docker Compose를 이용한 원클릭 수준의 배포 및 실행 환경 제공.
...
```
+-------------------------+       +---------------------------+       +---------------------+
|   Frontend (React/Vite) | <---->|    Backend (FastAPI)      | <---->|    LLM (Ollama)     |
|   (Port 3000)           |       |    (Port 8000)            |       | (e.g., qwq model)   |
|   - User Input          |       |    - API Endpoints        |       | - Text Generation   |
|   - Display Responses   |       |    - FAISS Search         |       +---------------------+
|   - User Feedback       |       |    - LLM Interaction      |                ^
|   - Document Viewer   |       |    - Feedback Logging     |                |
+-------------------------+       +---------------------------+                |
                                         |          ^                          |
                                         |          |                          |
                                         v          +--------------------------+
                            +--------------------+      +---------------------+
                            |   FAISS Index      |      | User Feedback       |
                            | (backend/faiss_index)|      | (backend/responses.jsonl)|
                            +--------------------+      +---------------------+
                                         ^
                                         |
                            +--------------------+
...
1.  **사용자**는 **프론트엔드(React/Vite)**를 통해 사고 정보를 입력합니다.
2.  **프론트엔드**는 정보를 **백엔드 API(FastAPI)**로 전송합니다.
3.  **백엔드**는 `database.csv`를 기반으로 생성된 **FAISS Index** (`backend/faiss_index/`)를 사용하여 유사 사례를 검색합니다.
4.  **백엔드**는 검색 결과와 사용자 입력을 조합하여 **Ollama (LLM)**에 대응 방안 생성을 요청합니다.
5.  **Ollama**는 두 가지 대응 방안 텍스트를 생성하여 **백엔드**로 반환합니다.
6.  **백엔드**는 생성된 방안을 **프론트엔드**로 전달합니다.
7.  **프론트엔드**는 사용자에게 두 방안을 제시하고, 사용자는 선택 또는 수정을 통해 피드백을 제공합니다.
8.  **프론트엔드**는 사용자 피드백(선택된 내용 또는 수정된 내용)을 **백엔드**로 전송합니다.
9.  **백엔드**는 수신된 피드백을 `responses.jsonl` 파일에 기록합니다.
10. (별도 기능) 사용자는 관련 문서 조회를 요청하고, **백엔드**는 해당 문서를 제공합니다.
...
3.  **LLM (Ollama)**
    ...
    *   **권장 모델**: `qwq` (또는 필요에 따라 다른 모델 사용 가능).
    *   **하드웨어 가속**: NVIDIA GPU(RTX 3090 등) 환경에서 CUDA를 통해 가속 지원.
    *   **모델 캐시**: Docker 볼륨(`ollama_data`) 또는 로컬 경로(`~/.ollama`)에 모델 파일 저장.

4.  **데이터 및 인덱스**
    *   `backend/database.csv`: RAG의 기반이 되는 원본 사고 사례 데이터.
    *   `backend/faiss_index/`: `buildRAGdb.py`를 통해 생성된 FAISS 인덱스 파일 저장 디렉토리. 빠른 유사도 검색에 사용됨.
...
*   **GPU**: NVIDIA GPU (RTX 3090 권장) - Ollama 및 FAISS-GPU 가속을 위해 필요.
*   **NVIDIA 드라이버 & CUDA Toolkit**: GPU 사용을 위한 드라이버 및 FAISS/PyTorch와 호환되는 CUDA 버전 설치 필요.
*   **Python**: 버전 3.12 이상.
*   **Node.js**: 버전 18 이상 권장 (Vite 호환성 확인 필요).
*   **npm** 또는 **yarn**: Node.js 패키지 매니저.
*   **Docker & Docker Compose**: 컨테이너 기반 실행을 위해 최신 버전 권장.
*   **Git**: 소스 코드 버전 관리 및 복제.
*   **RAM**: 최소 16GB 이상 권장 (LLM 모델 크기에 따라 더 많이 필요할 수 있음).
*   **디스크 공간**: 소스 코드, Docker 이미지, LLM 모델 파일, FAISS 인덱스를 저장할 충분한 공간.
...
### 1. 소스 코드 복제
```bash
git clone <repository_url> # 이 저장소의 URL 입력
cd <repository_name>       # 클론된 디렉토리로 이동
```
...
*   **RLHF 파이프라인 구체화**: `responses.jsonl` 데이터를 사용하여 주기적으로 LLM 파인튜닝 또는 RAG 시스템 평가/개선 프로세스 구축.
*   **데이터베이스 연동**: `database.csv`와 `responses.jsonl` 대신 PostgreSQL, MongoDB 등 DB 시스템을 도입하여 데이터 관리 효율성 및 확장성 증대.
*   **고급 RAG 전략**: 하이브리드 검색(키워드+벡터), Re-ranking 모델 도입, 동적 임베딩 업데이트 등 RAG 성능 향상 연구.
*   **임베딩/LLM 모델 최적화**: 더 가볍거나 성능이 우수한 모델 탐색 및 교체 실험 (Ko-LLM 모델 등).
*   **사용자 경험(UX) 개선**: 프론트엔드 인터페이스 개선, 응답 속도 최적화.
*   **사용자 인증 및 권한 관리**: 시스템 접근 제어 및 사용자별 데이터 관리 기능.
*   **모니터링 및 로깅 강화**: ELK 스택, Grafana 등 활용하여 시스템 상태, 성능, 오류 추적 시스템 구축.
